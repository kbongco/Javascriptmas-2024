...
function emojifyWord(word) {
  if (word.startsWith(':') && word.endsWith(':')) {
    const checkWord = word.slice(1, -1);
    return hackedEmojis[checkWord] || word;
}
}
...
function emojifyPhrase(phrase) {
  const emojiWord = phrase.match(/:\w+:/g);
  const emoji = emojifyWord(emojiWord[0]);
  return phrase.replace(emojiWord, emoji);
}
