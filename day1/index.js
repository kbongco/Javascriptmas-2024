...
const clearListButton = document.getElementById('clear-item-button')
const listArr = []
let listArrlen;
...
function checkDuplicate() {
  ...
  const itemText = itemInput.value.replace(/\s+/g, ' ').trim();
  if (!listArr.some(item => item.replace(/\s+/g, ' ').trim() === itemText)) {
...
function clearList(shoppingList) {
  shoppingList.innerHTML = '';
}
